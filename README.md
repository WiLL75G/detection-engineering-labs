...
**0Sricata IDS Custom Rules.** Suricata is deployed as a network sensor on the Ubuntu host, and two custom detection rules are written from scratch: a port scan detector and an SSH brute force detector. Both fire on live attacks from Kali. The SSH brute force rule detects the same attack that Wazuh caught host side in lab 01, demonstrating defense in depth across two independent sensors and two layers.
